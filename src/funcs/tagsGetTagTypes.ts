 ...
 */
export function tagsGetTagTypes(
  client: QlooCore,
  request?: operations.GetTagTypesRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetTagTypesResponse,
    | QlooError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetTagTypesRequest$outboundSchema.optional().parse(value),
  ...
  const path = pathToFunc("/tags/types")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetTagTypesResponse$inboundSchema),
    M.fail([400, "4XX"]),
    M.fail([500, "5XX"]),
  )(response, req);
  if (!result.ok) {
