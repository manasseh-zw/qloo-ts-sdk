 ...
 */
export function audiencesGetAudiences(
  client: QlooCore,
  request?: operations.GetAudiencesRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetAudiencesResponse,
    | QlooError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAudiencesRequest$outboundSchema.optional().parse(value),
  ...
  const path = pathToFunc("/audiences")();

  const query = queryJoin(
    encodeFormQuery({
      "filter.audience.types": payload?.["filter.audience.types"],
      "filter.parents.types": payload?.["filter.parents.types"],
      "filter.results.audiences": payload?.["filter.results.audiences"],
    }, { explode: false }),
    encodeFormQuery({
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAudiencesResponse$inboundSchema),
    M.fail([400, "4XX"]),
    M.fail([500, "5XX"]),
  )(response, req);
  if (!result.ok) {
