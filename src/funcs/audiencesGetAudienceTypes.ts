 ...
 */
export function audiencesGetAudienceTypes(
  client: QlooCore,
  request?: operations.GetAudienceTypesRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetAudienceTypesResponse,
    | QlooError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAudienceTypesRequest$outboundSchema.optional().parse(value),
  ...
  const path = pathToFunc("/audiences/types")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAudienceTypesResponse$inboundSchema),
    M.fail([400, "4XX"]),
    M.fail([500, "5XX"]),
  )(response, req);
  if (!result.ok) {
